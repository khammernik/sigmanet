...
class Discriminator(nn.Module):
    """ Discriminator for registration"""
...
def save_model(args, exp_dir, epoch, model, optimizer, D, D_opt, best_dev_loss, is_new_best, modelname='model.pt', save_epoch=False):
    ...
    torch.save(
        ...
        f=exp_dir / 'D_model.pt'
    )
    if is_new_best:
        shutil.copyfile(exp_dir / 'D_model.pt', exp_dir / 'D_best_model.pt')
    ...
    if save_epoch:
        ...
        torch.save(
            ...
            f=exp_dir / 'D_model_{}.pt'.format(epoch))

def postprocess_gan(tensor, shape):
...
def train_epoch(state, model, D_model, data_loader, optimizer, D_opt, writer):
    ...
    for iter, sample in enumerate(data_loader):
        ...
        if state.global_step % 1000 == 0:
            ...
            save_model(args, args.exp_dir, state.epoch, model, optimizer, D_model, D_opt,
...
def load_D_model(D_checkpoint_file):
    D_checkpoint = torch.load(D_checkpoint_file)
    D_model = Discriminator(in_channels=1).to(args.device)
    D_model.load_state_dict(D_checkpoint['model'])
    D_opt = torch.optim.Adam(D_model.parameters(), lr=1e-4, betas=(0.5, 0.999))
    D_opt.load_state_dict(D_checkpoint['optimizer'])
    return D_checkpoint, D_model, D_opt
...
def main(args):
    ...
    if args.checkpoint:
        ...
        # D_checkpoint, D_model, D_opt = load_D_model(args.D_checkpoint)
        if args.resume:
            # args = checkpoint['args']
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        save_model(args, args.exp_dir, epoch, model,
                   optimizer, D_model, D_opt, best_dev_loss, is_new_best, save_epoch=True)
        logging.info(
